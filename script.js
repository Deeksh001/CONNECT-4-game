...
document.getElementById('playerForm').addEventListener('submit', function (e) {
    ...
    document.getElementById('whosturn').innerText = `${playerColor === 'red' ? playerName : 'Computer'}'s Turn`;
...
function computerMove() {
    ...
    while (!validMove) {
        ...
        if (sum <= 6) {
            document.getElementById(`${col}r${sum}`).style.backgroundColor = computerColor;
            eval(`val_${col}++`);
            validMove = true;
            turn++;
            check(computerColor);
...
document.querySelectorAll(".column").forEach((e) => {
    e.addEventListener("click", () => {
        ...
        eval(`val_${e.id}++`);
        if (sum <= 6 && turn % 2 !== 0) {
            document.getElementById(`${e.id}r${sum}`).style.backgroundColor = playerColor;
            turn++;
            check(playerColor);
            document.getElementById("whosturn").innerText = `${playerColor === 'red' ? 'Computer' : playerName}'s Turn`;
